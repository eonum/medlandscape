    ...
    */
    dropdownSelectItem = (item) => {
		...
		this.props.fetchData(item).then(() => {
			console.log("enum Hospitals: ");
			console.log(this.props.hospitals[0]);
			this.setState({
				selectedEnum : item,
				selectedValues : [],
	...
	filter = () => {
		...
		if (hasLoaded) {
			...
			console.log("hospitals matching the filter: " + filteredHospitals.length);
    ...
    render () {
        return (
			<div className="control-panel">
				<DropdownMenu id="filterDropDown" listItems={this.state.variables} selectItem={this.dropdownSelectItem} selectedItem={this.state.selectedEnum} />
                {(this.state.selectedEnum !== undefined) ? <CheckboxList objects={this.state.selectedEnum.values} checkboxSelectItem={this.checkboxSelectItem} mappingObject={this.state.mappingObject} /> : null}
			</div>
        );
    }
}
