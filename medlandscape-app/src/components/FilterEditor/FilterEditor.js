			...
			let filteredHospitals = hospitals.filter(
				(item) => {
					...
					for(i = 0; i < this.state.selectedValues.length; i++){
						console.log(item, selectedYear, this.state.selectedVariable.name);
						const valueArray = item.attributes[this.state.selectedVariable.name][selectedYear]
							.split(", ");
						console.log(valueArray, this.state.selectedValues[i], valueArray.indexOf(this.state.selectedValues[i]));
